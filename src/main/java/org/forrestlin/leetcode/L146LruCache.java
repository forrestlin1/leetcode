...
class LRUCache1 {
    ...
    public void put(int key, int val) {
        // 先把新节点 x 做出来
        CacheNode x = new CacheNode(key, val);
        ...
        if (map.containsKey(key)) {
            ...
            if (cap == cache.size()) {
                // 删除链表最后一个数据
                CacheNode last = cache.removeLast();
                map.remove(last.key);
            }
            // 直接添加到头部
            cache.addFirst(x);
...
class DoubleList {
    ...
    private CacheNode head, tail; // 头尾虚节点
    ...
    // 在链表头部添加节点 x
    public void addFirst(CacheNode x) {
        x.next = head.next;
        x.prev = head;
        head.next.prev = x;
        head.next = x;
    ...
    // 删除链表中的 x 节点（x 一定存在）
    public void remove(CacheNode x) {
        x.prev.next = x.next;
        x.next.prev = x.prev;
        size--;
    }
